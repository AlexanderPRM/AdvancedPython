...
def get_doc_owner_name(user_doc_number=''):
    ...
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
            if doc_number == user_doc_number:
                return current_document['name'], True
    else:
...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    if doc_number not in directories[shelf_number]:
        ...
        print('Document already in shelf')
        return False
    ...
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
            if user_doc_number in directory_docs_list:
                return directory_number, True
    return 'Document not exist', False


def move_doc_to_shelf(user_doc_number='', user_shelf_number=''):
    if not user_doc_number:
        user_doc_number = input('Введите номер документа - ')
    if not user_shelf_number:
        user_shelf_number = input('Введите номер полки для перемещения - ')
    step1 = remove_doc_from_shelf(user_doc_number)
    step2 = append_doc_to_shelf(user_doc_number, user_shelf_number)
    if step1 and step2:
...
def show_document_info(document):
    doc_exist = check_document_existance(document)
    if doc_exist:
        for user_document in documents:
            if document == user_document['number']:
                doc_type = user_document['type']
                doc_number = user_document['number']
                doc_owner_name = user_document['name']
...
def add_new_doc(new_doc_number='', new_doc_type='', new_doc_owner_name='', new_doc_shelf_number=''):
    ...
    doc_exist = check_document_existance(new_doc_number)
    if not doc_exist:
        new_doc = {
            "type": new_doc_type,
        ...
        print(f'Document {new_doc_number} already created.')
        return False
